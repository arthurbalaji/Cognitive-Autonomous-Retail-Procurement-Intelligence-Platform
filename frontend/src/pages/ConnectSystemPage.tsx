...
const allProviders: ProviderDef[] = [
  ...
  {
    id: 'retailer-pos',
    name: 'CARPIP POS (Built-in)',
    description: 'Connect to the built-in POS application running alongside CARPIP. One-click setup — no external API keys needed.',
    icon: Store,
    color: '#10B981',
    forRole: 'RETAILER',
    fields: [
      { name: 'posApiUrl', label: 'POS API URL', type: 'url', placeholder: 'http://localhost:8081', helpText: 'Local dev: http://localhost:8081 · Docker: http://pos-backend:8081' },
      { name: 'posApiKey', label: 'API Key', type: 'text', placeholder: 'carpip-internal-key', helpText: 'Any non-empty value works for the internal POS (no auth required)' },
      { name: 'storeId', label: 'Store ID', type: 'text', placeholder: 'main-store', helpText: 'Store identifier (optional — defaults to "default")' },
    ],
    instructions: [
      'Ensure the POS application is running (port 8081)',
      'For local development, use http://localhost:8081 as the API URL',
      'For Docker deployment, use http://pos-backend:8081',
      'Enter any value for API Key (the internal POS does not require authentication)',
      'Click "Test & Connect" — inventory and sales data will sync automatically',
      '✅ The POS comes pre-loaded with sample products and transactions',
    ],
  },
  {
    id: 'shopify',
  ...
  },
  {
    ...
    forRole: 'RETAILER',
    fields: [
      { name: 'posApiUrl', label: 'Square API URL', type: 'url', placeholder: 'https://connect.squareup.com/v2', helpText: 'Square API base URL (use "http://demo" for test mode)' },
      { name: 'posApiKey', label: 'Access Token', type: 'password', placeholder: 'EAAAxxxxxxxxxxxxxxxxxxxxxxx', helpText: 'Found in Square Developer Dashboard → Applications → Credentials' },
  ...
  },
  {
    ...
    forRole: 'RETAILER',
    fields: [
      { name: 'posApiUrl', label: 'API Base URL', type: 'url', placeholder: 'https://api.your-pos.com/v1', helpText: 'Base URL for all API calls (use "http://demo" for test mode)' },
      { name: 'posApiKey', label: 'API Key / Bearer Token', type: 'password', placeholder: 'your-api-key-here', helpText: 'Authentication token sent as Bearer in Authorization header' },
  ...
  {
    id: 'wholesaler-erp',
    name: 'CARPIP ERP (Built-in)',
    description: 'Connect to the built-in ERP application running alongside CARPIP. One-click setup — no external API keys needed.',
    icon: Server,
    color: '#10B981',
    forRole: 'WHOLESALER',
    fields: [
      { name: 'erpApiUrl', label: 'ERP API URL', type: 'url', placeholder: 'http://localhost:8082', helpText: 'Local dev: http://localhost:8082 · Docker: http://erp-backend:8082' },
      { name: 'erpApiKey', label: 'API Key', type: 'text', placeholder: 'carpip-internal-key', helpText: 'Any non-empty value works for the internal ERP (no auth required)' },
      { name: 'warehouseId', label: 'Warehouse ID', type: 'text', placeholder: 'WH-MAIN', helpText: 'Warehouse identifier (optional — defaults to "WH-MAIN")' },
    ],
    instructions: [
      'Ensure the ERP application is running (port 8082)',
      'For local development, use http://localhost:8082 as the API URL',
      'For Docker deployment, use http://erp-backend:8082',
      'Enter any value for API Key (the internal ERP does not require authentication)',
      'Click "Test & Connect" — warehouse stock and sales orders will sync automatically',
      '✅ The ERP comes pre-loaded with 10 warehouse products and 8 confirmed sales orders',
    ],
  },
  {
    id: 'wholesaler-erp',
    ...
    forRole: 'WHOLESALER',
    fields: [
      { name: 'erpApiUrl', label: 'Service Layer URL', type: 'url', placeholder: 'https://your-sap-server:50000/b1s/v1', helpText: 'Your SAP Service Layer endpoint' },
      { name: 'erpApiKey', label: 'API Key / Session ID', type: 'password', placeholder: 'your-session-id', helpText: 'SAP session authentication token' },
  ...
  },
  {
    ...
    forRole: 'WHOLESALER',
    fields: [
      { name: 'erpApiUrl', label: 'API Base URL', type: 'url', placeholder: 'https://api.your-erp.com/v1', helpText: 'Base URL for all API calls (use "http://demo" for test mode)' },
      { name: 'erpApiKey', label: 'API Key / Bearer Token', type: 'password', placeholder: 'your-api-key-here', helpText: 'Authentication token sent as Bearer in Authorization header' },
      { name: 'inventoryEndpoint', label: 'Inventory Endpoint', type: 'text', placeholder: '/inventory', helpText: 'Relative path returning: [{sku, name, quantity, price}]' },
